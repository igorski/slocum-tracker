...
const SHARED_PATTERN_KEY  = ".OPTIMIZED_";
const PATTERN_CONTENT     = "    byte ";
const OUTPUT_FOLDER       = `${process.cwd()}/optimized`;
let INPUT_FOLDER;
...
function processDuplicates( duplicates ) {
    ...
    let sharedHeader = "; Shared header generated by Slocum Tracker Optimizer\n\n";
    sharedHeader += "    mac m_Music_SharedPatterns";
    ...
    console.log( `Shared header file for ${duplicates.length} duplicate pattern(s) created.` );
...
function updatePatternName( fileContents, patternNameToReplace, newPatternName ) {
    ...
    for ( let i = patternDeclarationIndex; i < textLines.length; ++i ) {
        textLines[ i ] = textLines[ i ].split( patternNameToReplace )
            .join( `; inline pattern declaration for Slocum Tracker import support. Generated by Slocum Tracker optimizer.\n${SHARED_PATTERN_KEY}${newPatternName}` );
    }
...
function getNameForSharedPattern( index ) {
    return `SHARED_Pattern_${index}T`;
}
